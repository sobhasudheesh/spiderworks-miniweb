...
use Spiderworks\MiniWeb\Models\Menu, Input, View, Redirect, DB, Datatables;
...
class MenuController extends BaseController
{
    ...
    protected function setDTData($collection) {
        ...
        return $this->initDTData($collection)
            ->editColumn('status', function($obj) use($route) { 
                if($obj->status == 1)
                {
                    return '<a href="' . route($route.'.change-status', [encrypt($obj->id)]).'" class="btn btn-success btn-sm miniweb-btn-warning-popup" data-message="Are you sure, want to disable this menu?"><i class="fa fa-check-circle"></i></a>'; 
                }
                else{
                    return '<a href="' . route($route.'.change-status', [encrypt($obj->id)]) . '" class="btn btn-danger btn-sm miniweb-btn-warning-popup" data-message="Are you sure, want to enable this menu?"><i class="fa fa-times-circle"></i></a>';
                }
            })
        ...
        if($menu_settings)
        {
            foreach ($menu_settings as $key => $setting) {
                ...
                if(isset($menu[$id]))
                {
                    ...
                    {
                    	$obj->linkable_type = "Spiderworks\MiniWeb\Models\Page";                     
                    ...
                    {
                        $obj->linkable_type = "Spiderworks\MiniWeb\Models\FrontendPage";                     
                        $obj->linkable_id = $menu[$id]['id'];
                    }
                    elseif($item_array[0] == 'category_link')
                    {
                        $obj->linkable_type = "Spiderworks\MiniWeb\Models\Category";                     
                        $obj->linkable_id = $menu[$id]['id'];
                    }
    ...
    {
        $items = MenuItem::where('menu_id', $menu_id)->where('parent_id', $parent)->get();
        if($items)
        {
